...
Vaccination data was acquired through the [CDC](https://covid.cdc.gov/covid-data-tracker/#vaccination-demographics-trends)
...
```{r message=FALSE, warning=FALSE}
hhs_data <- read_csv('data/COVID-19_Reported_Patient_Impact_and_Hospital_Capacity_by_State_Timeseries_20210607.csv')

census_data <- read.csv('data/sc-est2020-18+pop-res.csv')

state_abv <- read.delim2('data/state_abv.txt', sep = "|")

load("processed_data/vaccine_age_table.rda")
```
...
hhs_data <- hhs_data %>%
  ...
  ungroup() %>%
  filter(!collection_week >= "2021-06-04") # remove the latest week due to incomplete data collection at the initial time of running this script
```
...
First, we will stratify into pediatric and adult datasets
...
Modify the dates of the HHS and vaccination dataframes
...
vaccine_age_table <- vaccine_age_table %>% left_join(., date_week_id, by = "collection_week")
```
...
### **Vaccination Change Point **

```{r}
## adult vaccines
adult_vaccine <- vaccine_age_table %>% 
  select(collection_week, total_adult_week_stan) %>% 
  arrange(collection_week) %>%
  distinct(total_adult_week_stan)

adult_vaccine_mat <- t(data.matrix(adult_vaccine))
row.names(adult_vaccine_mat) <- NULL

## peds vaccines
peds_vaccine <- vaccine_age_table %>% 
  select(collection_week, total_pediatric_week_stan) %>% 
  arrange(collection_week) %>%
  distinct(total_pediatric_week_stan)

peds_vaccine_mat <- t(data.matrix(peds_vaccine))
row.names(peds_vaccine_mat) <- NULL


cp_nat_vac_adult <- change_point_test(adult_vaccine_mat, w = rep(1 / nrow(adult_vaccine_mat), nrow(adult_vaccine_mat)), 
                              t_range = 2:(ncol(adult_vaccine_mat) - 2), boot_num = 10000)


cp_nat_vac_ped <- change_point_test(peds_vaccine_mat, w = rep(1 / nrow(peds_vaccine_mat), nrow(peds_vaccine_mat)), 
                              t_range = 2:(ncol(peds_vaccine_mat) - 2), boot_num = 10000)
```
...
Note: t.min is not at the same scale as hospitalization trends, since vaccination data began 12/18/2020

```{r}
cp_nat_vac_adult <- unlist(cp_nat_vac_adult) %>% 
  data.frame()
colnames(cp_nat_vac_adult)[1] <- "National Adult Vaccinations"

cp_nat_vac_ped <- unlist(cp_nat_vac_ped) %>% 
  data.frame()
colnames(cp_nat_vac_ped)[1] <- "National Pediatric Vaccinations"

cp_nat_vac_results <- cbind(cp_nat_vac_adult, cp_nat_vac_ped) %>% t() %>% data.frame()

cp_nat_vac_results <- cp_nat_vac_results %>%
  mutate(p.value = round(p.value,4),
         stat = round(stat,1)) %>%
  t()

rownames(cp_nat_vac_results) <- c("Change Point Week", "P-value", "F-Stat")

datatable(cp_nat_vac_results)
```
...
We wil use bonferroni correction to adjust our p-value. We have 12 analyses, 2 for each region, 2 for the national level analysis, and 2 for the vaccination analysis.
...
**National Hospitalization Change points**

Note: Parameters to add vertical lines indicating vaccine change points (in progress!)
...
if(nrow(cp_adults_xy) == 0){
  
  cp_adults_xy[1,] <- NA
} 

# indicate vaccine change point dates
#vertical.lines <- c(as.Date("2021-03-05"), as.Date("2021-04-30"))
#vertical.lines.cols <- c("#1B9E77", "#D95F02")
...
ggplot(hhs_data_proc %>% 
  ...
  scale_x_date(date_labels = "%b", date_breaks = "1 month") + 
  scale_y_continuous(sec.axis = sec_axis(~ ./10, name = "Hospitalizations per 100K Children")) + 
  #geom_vline(xintercept = as.numeric(vertical.lines), linetype = "dashed", color = vertical.lines.cols) +
  theme_bw() + 
  theme(plot.title = element_text(""),
...
**Vaccination Change points**

```{r}
cp_vac_peds <- cp_nat_vac_results %>% 
  t() %>% 
  data.frame() %>% 
  tibble::rownames_to_column(., "Analysis") %>% 
  filter(P.value < sig_level, 
         grepl("Pediatric", Analysis)) %>% 
  distinct(Change.Point.Week) %>% 
  as.numeric()

cp_vac_adults <- cp_nat_vac_results %>% 
  t() %>% 
  data.frame() %>% 
  tibble::rownames_to_column(., "Analysis") %>% 
  filter(P.value < sig_level, 
...
cp_vac_peds_xy <- vaccine_age_table %>% 
  data.frame() %>% 
  distinct(week_id, collection_week, total_pediatric_week_stan) %>% 
  filter(week_id == cp_vac_peds)

if(nrow(cp_vac_peds_xy) == 0){
  
  cp_vac_peds_xy[1,] <- NA
} 

cp_vac_adults_xy <- vaccine_age_table %>% 
  data.frame() %>% 
  distinct(week_id, collection_week, total_adult_week_stan) %>% 
  filter(week_id == cp_vac_adults)

if(nrow(cp_vac_adults_xy) == 0){
  
  cp_vac_adults_xy[1,] <- NA
} 


ggplot(vaccine_age_table %>% 
         distinct(collection_week, week_id, total_pediatric_week_stan, total_adult_week_stan) %>% 
         mutate(total_pediatric_week_stan = total_pediatric_week_stan*5) %>% 
         pivot_longer(!c(collection_week, week_id), names_to = "population", values_to = "counts"))  +
       aes(x = collection_week, y = counts, group = population, color = population) + 
  geom_line() + 
  geom_point() + 
  ylab("Vaccinations per 100K Adults") + 
  xlab("") + 
  ...
  theme(plot.title = element_text(""),
        ...
        axis.title.y.right = element_text(face = "bold", size = 17, color='#D95F02', margin=margin(0,0,0,10)),
        axis.text.y.right = element_text(face = "bold", size = 17, color='#D95F02')) + 
  geom_text(data = data.frame(x = cp_vac_peds_xy$collection_week,
                              y = cp_vac_peds_xy$total_pediatric_week_stan*5, 
                              label = if_else(nrow(cp_vac_peds_xy) != 0,"*", NULL)),
            mapping = aes(x = x, y = y, label = label),
            size = 16, hjust = 0.5, vjust = 0.75, colour = "#D95F02", inherit.aes = FALSE) +
     geom_text(data = data.frame(x = cp_vac_adults_xy$collection_week,
                              y = cp_vac_adults_xy$total_adult_week_stan, 
                              label = if_else(nrow(cp_vac_adults_xy) != 0,"*", NULL)),
            mapping = aes(x = x, y = y, label = label),
            size = 16, hjust = 0.5, vjust = 0.75, colour = "#1B9E77", inherit.aes = FALSE)
```
