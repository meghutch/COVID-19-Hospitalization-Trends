...
```
"Sum of number of pediatric or patients who were admitted to an inpatient bed, including NICU, PICU, newborn, and nursery, on the previous calendar day who had confirmed COVID-19 at the time of admission."

```

We will also use the equivalent adult column: ```previous_day_admission_pediatric_covid_confirmed_7_day_sum```
...
hhs_data <- hhs_data %>%
  mutate(previous_day_admission_pediatric_covid_confirmed_7_day_sum = if_else(previous_day_admission_pediatric_covid_confirmed_7_day_sum < 0, 0, previous_day_admission_pediatric_covid_confirmed_7_day_sum),
         previous_day_admission_adult_covid_confirmed_7_day_sum = if_else(previous_day_admission_adult_covid_confirmed_7_day_sum < 0, 0, previous_day_admission_adult_covid_confirmed_7_day_sum))
```

**Select hospitals that at some point have a pediatric_count > 0 and select their unique hospital number** ```hospital_pk```

```{r}
pediatric_data <- hhs_data %>% 
  filter(previous_day_admission_pediatric_covid_confirmed_7_day_sum > 0) %>% 
...
```{r}
adult_data <- hhs_data %>% 
  filter(previous_day_admission_adult_covid_confirmed_7_day_sum > 0) %>% 
  distinct(hospital_pk) %>%
...
```{r}
hhs_pediatric_processed <- merge(hhs_data, pediatric_data, by = "hospital_pk")
hhs_adult_processed <- merge(hhs_data, adult_data, by = "hospital_pk")
```

**We will also standardize hospitalizations based on U.S Census population estimates**
...
national_stats <- hhs_data_proc %>% 
  ...
  group_by(population) %>%
  mutate(Q1 = round(quantile(hospitalizations_standardized_us, 0.25), 2),
         Median = round(median(hospitalizations_standardized_us), 2),
         Q3 = round(quantile(hospitalizations_standardized_us, 0.75), 2)) %>%
