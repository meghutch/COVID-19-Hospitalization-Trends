...
```{r message=FALSE, warning=FALSE}
hhs_data <- read_csv('data/COVID-19_Reported_Patient_Impact_and_Hospital_Capacity_by_State_Timeseries_20210325.csv')
...
To develop a new ```collection_week``` variable, we will download the older version of the HHS data and map each ```collection_week``` to a ```date_id```. We will then group each unique data in the newer version of the data into bins of 7 days that will match the previous old collection_week bins.

```{r message=FALSE, warning=FALSE}
old_release <- read_csv("data/reported_hospital_capacity_admissions_facility_level_weekly_average_timeseries_20210221.csv")
...
date_week_id <- old_release %>% distinct(collection_week) %>% arrange(collection_week)
...
hhs_data <- hhs_data %>%
  ...
  ungroup() %>%
  filter(!collection_week >= "2021-03-19") # remove the latest week due to incomplete data collection at the initial time of running this script
```
...
First, we will traify into pediatric and adult datasets
...
```{r}
save(hhs_data_proc, file = "data/hhs_data_proc.rda")
```
...
Import the [AAP pediatric and adult case counts](https://services.aap.org/en/pages/2019-novel-coronavirus-covid-19-infections/children-and-covid-19-state-level-data-report/)

```{r}
aap <- read.csv("data/processed_aap_case_data_2021.03.18.csv")

# change point functions
source("R/functions_to_share.R")
```

```{r}
## incorporate AAP 
# note: AAP data week means data was collected up to the end of the week 
# hhs week indicates the beginning of data collection
aap$Date <- as.Date(aap$Date)

aap <- aap %>% arrange(Date)

# remove the 730 date as this is before we have HHS data
aap <- aap %>% filter(!Date <= "2020-07-30")

## add sequence to week
aap$date_id <- seq.int(nrow(aap))
...
hhs_data_proc <- hhs_data_proc %>% left_join(., date_week_id, by = "collection_week") %>% 
  left_join(., aap, by = "date_id")

hhs_data_proc$Weekly_Pediatric_Case <- as.numeric(as.character(hhs_data_proc$Weekly_Pediatric_Case))

hhs_data_proc <- hhs_data_proc %>%
  mutate(Weekly_Pediatric_Case_standardized = Weekly_Pediatric_Case/pop_pediatric_nation*100000)
```


```{r}
ggplot(hhs_data_proc %>% 
         #mutate(hospitalizations_standardized_us = if_else(population == "pediatric", hospitalizations_standardized_us*15, hospitalizations_standardized_us)) %>% 
         #mutate(hospitalizations_standardized_us = hospitalizations_standardized_us*20) %>% 
         filter(!population == "pediatric") %>%
         distinct(collection_week, hospitalizations_standardized_us, population, Weekly_Pediatric_Case_standardized) %>%
         pivot_longer(cols = hospitalizations_standardized_us:Weekly_Pediatric_Case_standardized, 
                      names_to = "type", values_to = "count") %>%
         na.omit() %>% # 3/5/21 - missing AAP updated data
         mutate(population = as.character(population),
                population = if_else(type == "Weekly_Pediatric_Case_standardized", "x_cases", population)) %>%
         distinct(),
       aes(x = collection_week, y = count, color = population)) + 
  geom_point() + 
  #geom_area(aes(fill = "x_cases")) + 
  geom_line(aes(linetype = population)) +
  #geom_ribbon(data = . %>% filter(population == "x_cases"), stat ="identity", aes(fill = population, ymin = 0, ymax = count),alpha=0.6) +
  scale_color_brewer(palette="Dark2", labels = c("Adult Hospitalizations", "Child Cases"), 
                     name = "") + 
  ylab("Adult Hospitalizations / Child Cases") + 
  #ylab("Adult Hospitalizations\nChild Cases") + 
  xlab("") + 
  scale_x_date(date_labels = "%b-%d", date_breaks = "1 month") + 
  #scale_y_continuous(sec.axis = sec_axis(~ ./15, name = "Child Hospitalizations")) + 
  scale_linetype_manual(values=c("solid", "solid", "dashed")) +
  guides(linetype = FALSE,color=guide_legend(override.aes=list(linetype=c(1,2)))) + ## Drop the dummy legend ##
  theme_bw() 
```

### National Cases and Hospitalization Trends
...
```{r}
ped_cases <- hhs_data_proc %>% 
  ungroup() %>% 
  select(collection_week, Weekly_Pediatric_Case_standardized) %>% 
  arrange(collection_week) %>%
  distinct(Weekly_Pediatric_Case_standardized) %>%
  na.omit() 

ped_cases_mat <- t(data.matrix(ped_cases))
row.names(ped_cases_mat) <- NULL
...
```{r}
cp_nat_ped_cases <- change_point_test(ped_cases_mat, w = rep(1 / nrow(ped_cases_mat), nrow(ped_cases_mat)), 
                              t_range = 2:(ncol(ped_cases_mat) - 2), boot_num = 10000)
```

### Hospitalizations Change Point by Region

*We can’t do cases since we don’t have this data for pediatrics per region yet*
...
cp_nat_ped_cases <- unlist(cp_nat_ped_cases) %>% 
  data.frame()
colnames(cp_nat_ped_cases)[1] <- "National Pediatric Cases"

cp_nat_results <- cbind(cp_nat_adult, cp_nat_ped, cp_nat_ped_cases) %>% t() %>% data.frame()
